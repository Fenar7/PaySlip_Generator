  ...
  try {
    ...
    return new NextResponse(new Uint8Array(buffer), {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${buildInvoiceFilename(parsed.data.document, "png")}"`,
        "Cache-Control": "no-store",
      },
    });
  } catch (error) {
    console.error("Invoice PNG export failed", error);

    return NextResponse.json(
      {
        error: "Invoice PNG export failed.",
      },
      { status: 500 },
    );
  }
