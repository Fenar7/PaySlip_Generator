  ...
  return (
    <div className="space-y-6 text-[var(--voucher-ink)]">
      ...
      <div
        className={cn(
          "grid gap-4",
          isPrint ? "grid-cols-[1.15fr_0.85fr]" : "md:grid-cols-[1.15fr_0.85fr]",
        )}
      >
        <section className="rounded-[1.5rem] border border-[rgba(29,23,16,0.08)] bg-[rgba(255,255,255,0.86)] p-5">
          <div className={cn("grid gap-4", isPrint ? "grid-cols-2" : "sm:grid-cols-2")}>
            <div>
      ...
      {document.visibility.showSignatureArea ? (
        <section
          className={cn(
            "grid gap-4",
            isPrint ? "grid-cols-2" : "md:grid-cols-2",
          )}
        >
